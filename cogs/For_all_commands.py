...
class ForAll(cum.Cog):
    ...
    async def randomp(self, ctx):
        ...
        emb.set_footer(text="Вы можете добавить собственную фотографию, написав <sendp и указав на нее ссылку",
                       icon_url=ctx.author.avatar)
        emb.set_image(url=roflophotos.photos[random.randint(0, len(roflophotos.photos) - 1)])
        await ctx.send(embed=emb)

    @cum.command()
    async def sendp(self, ctx, url1):
        print(url1)
        db2 = sqlite3.connect("photos.sqlite")
        cursor2 = db2.cursor()
        sql = "INSERT INTO urls(url) VALUES (?)"
        cursor2.execute(sql, (url1))
        db2.commit()
        cursor2.close()
        db2.close()
